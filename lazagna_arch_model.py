...
class Port:
    name: str
    num_pins: int = 1  # most ports are single-pin, so omitting it in XML is fine
...
LayerDirective = SingleLayout | ColLayout | RowLayout | FillLayout | PerimeterLayout | RegionLayout


def _grid_to_singles(grid: list[list[list[str]]], priority: int = 10) -> list[SingleLayout]:
    directives: list[SingleLayout] = []
    for layer in grid:
        for row_idx, row in enumerate(layer):
            for col_idx, block_type in enumerate(row):
                if block_type:
                    directives.append(SingleLayout(
                        type=block_type,
                        x=col_idx,
                        y=row_idx,
                        priority=priority,
...
class FixedLayout:
    ...
    def add_directive(self, directive: LayerDirective) -> FixedLayout:
        self.directives.append(directive)
        return self

    def from_grid(self, grid: list[list[list[str]]], priority: int = 10) -> FixedLayout:
        self.directives.extend(_grid_to_singles(grid, priority))
        return self
...
class Device:
    ...
    default_fc_in_val: float = 0.15   # fraction of routing tracks each block input can reach
...
class Segment:
    ...
    def to_xml(self) -> ET.Element:
        ...
        elem.set("type", self.type)
        # sb and cb are child elements in the VTR spec, not attributes
        sb = ET.SubElement(elem, "sb")
        sb.set("type", self.sb_pattern)
        cb = ET.SubElement(elem, "cb")
        cb.set("type", self.cb_pattern)
        if self.mux_name:
...
class Architecture:
    ...
    def _infer_tiles(self) -> list[ET.Element]:
        ...
        for pb in self.complexblocklist:
            ...
            t.set("height", "1")
            t.set("area", "0.0")
            eq = ET.SubElement(t, "equivalent_sites")
    ...
    def to_xml(self) -> str:
        ...
        if self.complexblocklist:
            ...
            for pb in self.complexblocklist:
                cbl.append(pb.to_xml())

        ET.indent(root, space="  ")
        return ET.tostring(root, encoding="unicode", xml_declaration=False)


# stub for now, fields need to be filled in once I get a look at the actual YAML
@dataclass
class ExperimentOptions:
    experiment_name: str = "default_experiment"
    route_chan_width: Optional[int] = None
    place_algorithm: str = "path_timing_driven"
    analysis: bool = True
    extra: dict = field(default_factory=dict)

    def to_dict(self) -> dict:
        d: dict = {"experiment_name": self.experiment_name,
                   "place_algorithm": self.place_algorithm,
                   "analysis": self.analysis}
        if self.route_chan_width is not None:
            d["route_chan_width"] = self.route_chan_width
        d.update(self.extra)
        return d


def run_lazagna(arch: Architecture, opts: ExperimentOptions) -> dict:
    # will write arch XML + opts YAML to temp files, call the runner, return results
    raise NotImplementedError("Wire up to the LaZagna runner in Phase 2.")


if __name__ == "__main__":
    lut4 = PbType.primitive(
        name="lut4",
        blif_model=".names",
        inputs=[Port("in", num_pins=4)],
    ...
    clb = PbType.container(
        ...
        children=[lut4, ff],
        interconnect=[
            Direct("lut_in",  input="clb.I[0:3]",  output="lut4.in[0:3]"),
            Direct("lut_out", input="lut4.out[0]", output="ff.D[0]"),
            Direct("ff_out",  input="ff.Q[0]",     output="clb.O[0]"),
        ],
    )

    iopad = PbType.primitive(
        name="iopad",
        blif_model=".io",
        inputs=[Port("outpad")],
        outputs=[Port("inpad")],
    ...
    io = PbType(
        ...
        modes=[
            Mode(
                name="inpad",
                children=[iopad],
                interconnect=[Direct("io_inpad", input="iopad.inpad", output="io.inpad")],
            ),
            Mode(
                name="outpad",
                children=[iopad],
                interconnect=[Direct("io_outpad", input="io.outpad", output="iopad.outpad")],
            ),
            Mode(
                name="physical",
                children=[iopad],
                interconnect=[
                    Direct("io_outpad", input="io.outpad", output="iopad.outpad"),
                    Direct("io_inpad",  input="iopad.inpad", output="io.inpad"),
                ],
    ...
    fixed = (
        FixedLayout(name="main", width=4, height=4)
        .from_grid(grid, priority=10)
    )

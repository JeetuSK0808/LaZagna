...
# The add_layer() calls are just so you never have to touch SingleLayout directly.
@dataclass
class SingleLayout:
    type: str
    priority: int = 1
    ...
    def to_xml(self) -> ET.Element:
        ...
        _set_attrs(elem, self, "x", "y", "startx", "starty",
                   "repeatx", "repeaty", "incrx", "incry")
        return elem
...
class FixedLayout:
    ...
    width: int
    height: int
    layers: list[SingleLayout] = field(default_factory=list)

    def add_layer(self, tile_type: str, **kwargs) -> FixedLayout:
        self.layers.append(SingleLayout(type=tile_type, **kwargs))
        return self
...
class AutoLayout:
    ...
    def add_layer(self, tile_type: str, **kwargs) -> AutoLayout:
        self.layers.append(SingleLayout(type=tile_type, **kwargs))
        return self
...
if __name__ == "__main__":
    ...
    arch = Architecture(
        layout=Layout(
            fixed=FixedLayout(name="main", width=8, height=8)
                .add_layer("io",  x="0", y="0", priority=2)
                .add_layer("clb", startx="1", starty="1", repeatx="1", repeaty="1", priority=1)
        ),
        device=Device(
